...
from terabox import TeraBoxAPI, extract_surl, is_terabox_link
from utils import (
...
# ── In-memory state ───────────────────────────────────────────────────────────
# user_id → set of asyncio.Task (active downloads)
active_tasks: dict[int, set] = defaultdict(set)
# user_id → asyncio.Event (cancel signal)
cancel_events: dict[int, asyncio.Event] = {}
# Premium users (user_ids with higher parallel limit)
premium_users: set = set()
# user_id → (surl, original_text, status_msg) — waiting for password input
password_pending: dict = {}
# user_id → (surl, fs_id, uk, share_id, dl_path, file_info, msg) — waiting for rename input
rename_pending: dict = {}


def get_user_limit(user_id: int) -> int:
    """Return concurrent download limit for this user."""
...
async def check_banned(msg_or_cq) -> bool:
    """Returns True if user is banned (and sends message)"""
...
async def check_force_join(msg: Message) -> bool:
    """Returns True if user has NOT joined the required channel"""
...
SPLIT_SIZE = 2 * 1024 * 1024 * 1024  # 2GB split threshold


async def split_file(path: str, part_size: int = SPLIT_SIZE) -> list[str]:
    """
    Split a large file into part_size chunks.
...
async def send_file_or_split(
    ...
    """
    Send file normally if <= 2GB.
    Auto-split and send as parts if > 2GB.
...
"
            ...
            # Use kuri for large parts if available
            uploader = uploader_kuri if uploader_kuri else client

            # Rename part file so Telegram shows correct name
...
def _build_url_from_surl(surl: str) -> str:
    """Reconstruct a share URL for the backend API"""
...
async def cmd_help(_, msg: Message):
    await msg.reply(
        ...
        "4️⃣ Tap **Download** → file sent!\n\n"
        "**📁 Folder links:**\n"
        "Bot lists all files. Tap any to download, or **Download All**.\n\n"
        "**🎯 Custom Channel/Group:**\n"
        "Use /setchat to set where bot uploads.\n\n"
        "**⏳ Auto Delete:**\n"
        f"Files auto-deleted after {Config.AUTO_DELETE // 60} minutes.\n\n"
        "**📏 File limits:**\n"
        "• Up to 2 GB → Bot API\n"
        "• Up to 4 GB → MTProto (Kurigram)\n\n"
        "**🎬 Video quality:**\n"
        "1080p → 720p → 480p (auto fallback)\n\n"
        "**⛔ Cancel:**\n"
...
async def cmd_setchat(_, msg: Message):
    ...
    try:
        ...
        await msg.reply("❌ Chat not found! Make sure ID is correct and bot is in that chat.")
...
async def cmd_mychat(_, msg: Message):
    ...
    try:
        ...
        await msg.reply(f"🎯 Target ID: `{target}` _(chat info unavailable)_")
...
async def cmd_ban(_, msg: Message):
    ...
    await msg.reply(f"✅ User `{uid}` has been **banned**.\nReason: `{reason or 'None'}`")
...
async def cmd_unban(_, msg: Message):
    ...
    await msg.reply(f"✅ User `{uid}` has been **unbanned**.")
...
# ─────────────────────────────────────────────────────────────────────────────
#  PASSWORD HANDLER — intercepts password replies for protected links
# ─────────────────────────────────────────────────────────────────────────────

@bot.on_message(filters.text & filters.private)
async def handle_password(_, msg: Message):
    """If user is in password_pending state, treat their message as the password."""
    user_id = msg.from_user.id
    if user_id not in password_pending:
        return  # Not waiting for password — let handle_link process it
...
async def handle_link(_, msg: Message):
    ...
    user_id = msg.from_user.id
    # Skip if user is currently entering a password
...
async def _show_file(msg: Message, f: dict, surl: str, orig_url: str, uk, share_id):
    ...
    cb_data = f"dl|{surl}|{fs_id}|{uk or ''}|{share_id or ''}"
    rename_data = f"rename|{surl}|{fs_id}|{uk or ''}|{share_id or ''}"
    btns = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"📥 Download  ({humanbytes(size)})", callback_data=cb_data)],
        [InlineKeyboardButton("✏️ Rename & Download", callback_data=rename_data)],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel")],
    ])

    if thumb:
        try:
            await msg.reply_photo(thumb, caption=txt, reply_markup=btns)
            await msg.delete()
            return
        except Exception:
            pass
...
async def _show_folder(msg: Message, files: list, surl: str, orig_url: str, uk, share_id):
    ...
    txt = (
        ...
        f"Tap a file to download individually, or **Download All**:\n"
    )
    ...
    # Download All button (max 10 files to avoid Telegram limits)
    if 1 < total <= 10:
...
async def cb_download(client: Client, cq: CallbackQuery):
    ...
    try:
        ...
        # ── Resolve best download URL (Step 3/4/5) ───────────────────────────
        dl_url, is_m3u8, quality = await api.resolve_download(
            surl, file_info, uk, share_id
        )
        if not dl_url:
            await msg.edit("❌ Could not get download URL!\nLink may require login or is expired.")
            return
        ...
        # ── Upload (with auto-split for >2GB) ───────────────────────────────
        target_chat = await db.get_target_chat(user_id) or msg.chat.id
        await msg.edit(f"📤 **Uploading...**\n\n📄 `{name}`")

        bot_me = await client.get_me()
        u = cq.from_user
        quality_label = "Streaming quality (same content in less MB without compression)" if is_m3u8 else "Original quality"
        ...
        caption += (
            f"⬇️ **Downloaded in:** `{fmt_hms(dl_time)} sec`\n"
            f"⬆️ **Uploaded in:** `{fmt_hms(up_time)} sec`\n"
            f"👤 **Uploaded by:** `{u.id}`\n\n"
        )
        if auto_del_min:
            caption += (
                f"⚠️ This file will auto-delete in `{auto_del_min} minutes`.\n"
                f"📨 Please forward it to save permanently.\n\n"
            )
...
async def cb_download_all(client: Client, cq: CallbackQuery):
    ...
    msg = cq.message
    await msg.edit(f"📦 **Bulk Download Started!**\nDownloading {len(fs_ids)} files one by one...")
...
# ─────────────────────────────────────────────────────────────────────────────
#  RENAME CALLBACK — user taps "Rename & Download"
# ─────────────────────────────────────────────────────────────────────────────
...
async def handle_rename_input(client: Client, msg: Message):
    """Intercept rename input before other handlers."""
    user_id = msg.from_user.id
...
✏️ Will rename to: `{new_name}`")
    ...
    try:
        ...
        # Override filename with user's new name
...
"
                f"📦 **Size:** `{humanbytes(actual_size)}`
"
                f"✏️ **Renamed from:** `{file_info.get('server_filename', '?')}`
"
...
async def cb_help(_, cq: CallbackQuery):
    ...
    await cq.message.reply(
        "📖 Send me a TeraBox / Nephobox link and I'll handle the rest!\n"
...
async def main():
    ...
    if Config.LOG_CHANNEL:
        try:
            await bot.send_message(Config.LOG_CHANNEL, "🚀 **TeraBox Bot v5.0 started!**")
        except Exception:
            pass
...
async def cmd_add_premium(_, msg: Message):
    ...
    await msg.reply(
        f"⭐ User `{uid}` is now **Premium**!\n"
        f"Parallel limit: `{Config.MAX_CONCURRENT_PREMIUM}` downloads"
    )
...
async def cmd_rem_premium(_, msg: Message):
    ...
    await msg.reply(
        ...
        f"Limit reset to: `{Config.MAX_CONCURRENT}` downloads"
    )


@bot.on_message(filters.command("premiumlist") & filters.user(Config.ADMIN_IDS))
async def cmd_premium_list(_, msg: Message):
    """Show all current premium users."""
    if not premium_users:
        return await msg.reply("📋 No premium users yet.")
    txt = "⭐ **Premium Users:**\n\n"
    for uid in premium_users:
        txt += f"• `{uid}`\n"
    txt += f"\n**Limit:** `{Config.MAX_CONCURRENT_PREMIUM}` parallel downloads"
    await msg.reply(txt)


@bot.on_message(filters.command("mylimit") & filters.private)
async def cmd_my_limit(_, msg: Message):
    """Show user's current download limit."""
    uid = msg.from_user.id
