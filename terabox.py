...
class TeraBoxAPI:
    ...
    async def resolve_download(
        ...
        quality param:
          None / "AUTO"         → best available (backend stream-app first)
          "M3U8_AUTO_1080"      → 1080p HLS
          "M3U8_FLV_264_720"    → 720p HLS
          "M3U8_FLV_264_480"    → 480p HLS
          "M3U8_FLV_264_360"    → 360p HLS
        force_direct: True → skip HLS, use direct download only
        """
        ...
        # ── Force direct download (user selected Original) ────────────────────
        if force_direct:
            dlink = await self.get_direct_link(surl, fs_id, uk, share_id, md5)
            if dlink:
                return dlink, False, "Original"
            if file_info.get("dlink"):
                return file_info["dlink"], False, "Original"
            return None, False, ""

        # ── Specific quality requested ────────────────────────────────────────
        if quality and quality not in (None, "AUTO") and is_vid and uk and share_id:
            m3u8 = await self.get_stream_url(surl, fs_id, uk, share_id, quality=quality)
            if m3u8:
                from utils import fmt_quality_label
                label = {
                ...
                return m3u8, True, label
            # Fallback to direct if specific quality not available
            dlink = await self.get_direct_link(surl, fs_id, uk, share_id, md5)
            if dlink:
                return dlink, False, "Original (fallback)"
            return None, False, ""

        # ── Auto (best quality) ───────────────────────────────────────────────
        # 1. M3U8 stream for video via backend (sign handled server-side)
        if is_vid and uk and share_id:
            m3u8, qlabel = await self.get_best_stream(surl, fs_id, uk, share_id, md5)
            if m3u8:
                return m3u8, True, qlabel or "Auto"
